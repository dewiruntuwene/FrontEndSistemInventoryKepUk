        ...
        <table class="table-auto border-collapse border border-gray-300 w-full text-sm mt-6">
          <thead>
            <tr>
              ...
              <th class="border border-gray-300 px-4 py-2">Barang Sudah di Terima</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="(barang, barangIndex) in container.barang" :key="barangIndex">
              ...
              <td class="border border-gray-300 px-4 py-2 text-center align-middle">
                  ...
                  <span v-if="barang.sudah_dibeli === 'NO'" class="text-red-500 font-bold">
                    NO
                  </span>
              ...
              <td class="border border-gray-300 px-4 py-2 text-center align-middle">
                  <!-- Jika sudah_dibeli == "NO", tampilkan tombol -->
                  <span v-if="barang.sudah_diterima === 'NO'">
                  <button
                      class="bg-blue-500 text-white px-2 py-1 rounded hover:bg-blue-600"
                      @click="updateSudahDiterima(barang.id_barang_preorder)"
                  >
                      Update to YES
                  </button>
                  </span>
...
const fetchOrders = async () => {
  ...
  try {
    const response = await axios.get(`${apiUrl}/barangpreoderapprovedadmin`, {
...
const updateSudahDiterima = async (id_barang_preorder: number) => {
    const token = localStorage.getItem("token");
    try {
        await axios.patch(
        `${apiUrl}/barangsudahditerima/${id_barang_preorder}`,
        { sudah_diterima: "YES" },
        { headers: { Authorization: `Bearer ${token}` } }
        );
        fetchOrders();
