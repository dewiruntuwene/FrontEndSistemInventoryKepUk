        ...
        <div class="overflow-x-auto mx-auto max-w-4xl mr-10 min-w-3.5 h-fit pt-2">
          <table class="min-w-full divide-y  divide-gray-200 justify-center">
            ...
            <thead class="bg-blue-200">
              <tr>
                <th scope="col" class="px-2 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider border-b text-center">
                  Id Peminjam
                </th>
                <th scope="col" class="px-2 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider border-b text-center">
                  Nama Dosen
            ...
            <tbody class="bg-white divide-y divide-gray-200 mr-mx">
              ...
              <tr v-for="(transaction, index) in transactions" :key="index">
                ...
                <td class="px-2 py-2 whitespace-nowrap">
                  <div class="text-xs text-gray-900">{{ transaction.id_peminjam }}</div>
                </td>
                <td class="px-2 py-2 whitespace-nowrap">
                  <div class="text-xs text-gray-900">{{ transaction.users.username }}</div>
                </td>
                <td class="px-2 py-2 whitespace-nowrap">
                  <div class="text-xs text-gray-900">{{ transaction.nama_matakuliah }}</div>
                </td>
                <td class="px-2 py-2 whitespace-nowrap">
                  <div class="text-xs text-gray-900">{{ transaction.prasat }}</div>
                </td>
                <td class="px-2 py-2 whitespace-nowrap">
                  <div class="text-xs text-gray-900">{{ transaction.jam_praktek }}</div>
                </td>
                <td class="px-2 py-2 whitespace-nowrap">
                  <div class="text-xs text-gray-900">{{ transaction.tanggal_pengambilan }}</div>
                </td>
                <td class="px-2 py-2 whitespace-nowrap">
                  <div class="text-xs text-gray-900">
                    <ul>
                      <li v-for="(item, index) in transaction.barangPinjam" :key="index">
                        {{ item.nama_barang }} ({{ item.jenis_barang }}) - {{ item.jumlah_barang }}
                      </li>
                    </ul>
                  </div>
                </td>
                <td class="px-2 py-2 whitespace-nowrap text-center">
                  <template v-if="transaction.status === 'pending'">
                    <button @click="updateTransaction(Number(transaction.id_peminjam))" type="button" class="focus:outline-none text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800">Selesai</button>
                    <button @click="updateTransaction(Number(transaction.id_peminjam))" type="button" class="focus:outline-none text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900">Ditolak</button>
                  </template>
                  <template v-else>
