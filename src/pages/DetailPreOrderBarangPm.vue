  ...
  <div class="container mx-auto py-4 mr-px">
    ...
    <div v-for="(container, index) in ordersGroupedByContainer" :key="index" class="card border border-gray-300 mb-6 rounded shadow">
      ...
      <div class="p-4 bg-gray-100">
        ...
        <p>Status Warek 1: 
          <span :class="container.status_pm === 'PENDING' ? 'text-red-500' : 'text-green-500'">
            {{ container.status_pm }}
          </span>
        </p>
        <!-- Tampilkan tombol update jika status warek 1 masih PENDING -->
        <button
          v-if="container.status_pm === 'PENDING'"
          class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600 mt-2"
          @click="updateStatusWarek1(container.id_container)"
        >
          Update Status
        </button>
      </div>
...
import LayoutPm from "../components/LayoutPm.vue";
...
const fetchOrders = async () => {
  ...
  try {
    const response = await axios.get(`${apiUrl}/barangpreoderapprovedpm`, {
...
const updateStatusWarek1 = async (idContainer: number) => {
  const token = localStorage.getItem("token");
  try {
    await axios.patch(`${apiUrl}/databarangpreorderpm/${idContainer}`, {
